...
def createIndex(connection, tableName, columnName, unique=False):
    ...
    try:
        ...
        if unique:
            s += "UNIQUE "
        cursor.execute(s + "INDEX IF NOT EXISTS " + tableName + "_" +
                       columnName + "_index ON " + tableName + "(" + columnName + ")")
        connection.commit()
        # connection.close()
    except sqlite3.OperationalError as error:
        printDBError(error)
...
def getMinMax(connection, tableName, columnName, minmax):
    ...
    try:
        ...
        if minmax == 'min' or minmax == 'max':
            cursor.execute(
                "SELECT " + minmax + "(" + columnName + ") from " + tableName)
        elif minmax == 'minmax':
            cursor.execute(
                "SELECT MIN(" + columnName + "), MAX(" + columnName + ") from " + tableName)
        else:
        ...
        if type(objectNumbers) == int and objectNumbers == -1:
            for trajectory in objects:
                ...
                for position in trajectory.getPositions():
                    ...
                    cursor.execute(
                        query, (trajectory_id, frame_number, position.x, position.y))
...
def getObjectCriteria(objectNumbers):
    if objectNumbers is None:
        ...
        print('objectNumbers {} are not a known type ({})'.format(
            objectNumbers, type(objectNumbers)))
        query = ''
    return query
    ...
    for row in cursor:
        if row[0] != objId:
            ...
            if obj is not None and obj.length() == obj.positions.length():
                ...
                print('Object {} is missing {} positions'.format(
                    obj.getNum(), int(obj.length()) - obj.positions.length()))
            obj = moving.MovingObject(row[0], timeInterval=moving.TimeInterval(
                row[1], row[1]), positions=moving.Trajectory([[row[2]], [row[3]]]))
        else:
...
def loadTrajectoriesFromSqlite(filename, trajectoryType, objectNumbers=None, withFeatures=False):
    ...
    objects = loadTrajectoriesFromTable(
        connection, 'positions', trajectoryType, objectNumbers)
    objectVelocities = loadTrajectoriesFromTable(
        connection, 'velocities', trajectoryType, objectNumbers)
    ...
    if trajectoryType == 'object':
        ...
        try:
            ...
            if objectNumbers is not None:
                queryStatement += ' and OF.object_id ' + objectCriteria
            # order is important to group all features per object
            queryStatement += ' group by P.trajectory_id order by OF.object_id'
            cursor.execute(queryStatement)
            logging.debug(queryStatement)
            ...
            # load userType
            userTypes = loadUserTypesFromTable(
                cursor, trajectoryType, objectNumbers)
            for obj in objects:
            ...
            if withFeatures:
                ...
                features = loadTrajectoriesFromSqlite(
                    filename, 'feature', nFeatures + 1)
                for obj in objects:
...
def savePrototypesToSqlite(filename, prototypes, trajectoryType='feature'):
    ...
    try:
        cursor.execute(
            'CREATE TABLE IF NOT EXISTS prototypes (id INTEGER PRIMARY KEY, object_id INTEGER, trajectory_id INTEGER, nMatchings INTEGER, FOREIGN KEY(object_id) REFERENCES objects(id), FOREIGN KEY(trajectory_id) REFERENCES positions(trajectory_id))')
...
def loadGroundTruthFromSqlite(filename, gtType='bb', gtNumbers=None):
    ...
    if gtType == 'bb':
        topCorners = loadTrajectoriesFromTable(
            connection, 'bounding_boxes', 'bbtop', gtNumbers)
        bottomCorners = loadTrajectoriesFromTable(
            connection, 'bounding_boxes', 'bbbottom', gtNumbers)
        # string format is same as object
        userTypes = loadUserTypesFromTable(
            connection.cursor(), 'object', gtNumbers)

        for t, b in zip(topCorners, bottomCorners):
            num = t.getNum()
            if t.getNum() == b.getNum():
                annotation = moving.BBAnnotation(
...
def deleteFromSqlite(filename, dataType):
    ...
    if os.path.isfile(filename):
        ...
        if dataType == 'object':
            ...
            print(
                'Unknown data type {} to delete from database'.format(dataType))
        connection.close()
    else:
...
def createInteractionTable(cursor):
    cursor.execute(
        'CREATE TABLE IF NOT EXISTS interactions (id INTEGER PRIMARY KEY, object_id1 INTEGER, object_id2 INTEGER, first_frame_number INTEGER, last_frame_number INTEGER, FOREIGN KEY(object_id1) REFERENCES objects(id), FOREIGN KEY(object_id2) REFERENCES objects(id))')
...
def createIndicatorTables(cursor):
    ...
    cursor.execute(
        'CREATE TABLE IF NOT EXISTS indicators (interaction_id INTEGER, indicator_type INTEGER, frame_number INTEGER, value REAL, FOREIGN KEY(interaction_id) REFERENCES interactions(id), PRIMARY KEY(interaction_id, indicator_type, frame_number))')


def saveInteraction(cursor, interaction):
    roadUserNumbers = list(interaction.getRoadUserNumbers())
    cursor.execute('INSERT INTO interactions VALUES({}, {}, {}, {}, {})'.format(interaction.getNum(
    ), roadUserNumbers[0], roadUserNumbers[1], interaction.getFirstInstant(), interaction.getLastInstant()))
    ...
    try:
        ...
        for row in cursor:
            if row[0] != interactionNum:
                ...
                interactions.append(events.Interaction(
                    interactionNum, moving.TimeInterval(row[3], row[4]), row[1], row[2]))
                interactions[-1].indicators = {}
            if indicatorTypeNum != row[5] or row[0] != interactionNum:
                indicatorTypeNum = row[5]
                indicatorName = events.Interaction.indicatorNames[
                    indicatorTypeNum]
...
# from positions P, objects_features OF WHERE P.trajectory_id =
# OF.trajectory_id group by OF.object_id order by OF.object_id
...
def loadBoundingBoxTableForDisplay(filename):
    ...
    try:
        cursor.execute(
            'SELECT name FROM sqlite_master WHERE type=\'table\' AND name=\'bounding_boxes\'')
        result = [row for row in cursor]
        if len(result) > 0:
            cursor.execute('SELECT * FROM bounding_boxes')
            for row in cursor:
                boundingBoxes.setdefault(row[1], []).append(
                    [moving.Point(row[2], row[3]), moving.Point(row[4], row[5])])
    except sqlite3.OperationalError as error:
        printDBError(error)
...
def writeFeaturesToSqlite(objects, outputFilename, trajectoryType, objectNumbers=-1):
    ...
    cursor.execute(
        "CREATE TABLE IF NOT EXISTS \"positions\"(trajectory_id INTEGER,frame_number INTEGER, x_coordinate REAL, y_coordinate REAL, PRIMARY KEY(trajectory_id, frame_number))")
    cursor.execute(
        "CREATE TABLE IF NOT EXISTS \"velocities\"(trajectory_id INTEGER,frame_number INTEGER, x_coordinate REAL, y_coordinate REAL, PRIMARY KEY(trajectory_id, frame_number))")
...
def writePrototypesToSqlite(prototypes, nMatching, outputFilename):
    ...
    cursor.execute(
        "CREATE TABLE IF NOT EXISTS \"prototypes\"(prototype_id INTEGER,routeIDstart INTEGER,routeIDend INTEGER, nMatching INTEGER, PRIMARY KEY(prototype_id))")
...
def writeLabelsToSqlite(labels, outputFilename):
    ...
    cursor.execute(
        "CREATE TABLE IF NOT EXISTS \"labels\"(object_id INTEGER,routeIDstart INTEGER,routeIDend INTEGER, prototype_id INTEGER, PRIMARY KEY(object_id))")
...
def writeSpeedPrototypeToSqlite(prototypes, nmatching, outFilename):
    ...
    cursor.execute(
        "CREATE TABLE IF NOT EXISTS \"speedprototypes\"(spdprototype_id INTEGER,prototype_id INTEGER,routeID_start INTEGER, routeID_end INTEGER, nMatching INTEGER, PRIMARY KEY(spdprototype_id))")
...
def writeRoutesToSqlite(Routes, outputFilename):
    ...
    cursor.execute(
        "CREATE TABLE IF NOT EXISTS \"routes\"(object_id INTEGER,routeIDstart INTEGER,routeIDend INTEGER, PRIMARY KEY(object_id))")
...
def setRoutes(filename, objects):
    ...
    cursor = connection.cursor()
    for obj in objects:
        cursor.execute('update objects set startRouteID = {} WHERE object_id = {}'.format(
            obj.startRouteID, obj.getNum()))
        cursor.execute('update objects set endRouteID = {} WHERE object_id = {}'.format(
            obj.endRouteID, obj.getNum()))
    connection.commit()
    connection.close()
...
def setRoadUserTypes(filename, objects):
    ...
    cursor = connection.cursor()
    for obj in objects:
        cursor.execute('update objects set road_user_type = {} WHERE object_id = {}'.format(
            obj.getUserType(), obj.getNum()))
    connection.commit()
    connection.close()
...
def convertTrajectoriesVissimToSqlite(filename):
    ...
    check_call("sqlite3 " + utils.removeExtension(filename) +
               ".sqlite < " + sqlScriptFilename, stderr=out, shell=True)
    out.close()
...
def loadObjectNumbersInLinkFromVissimFile(filename, linkIds):
    ...
    connection = sqlite3.connect(filename)
    cursor = connection.cursor()
    queryStatement = 'SELECT DISTINCT trajectory_id FROM curvilinear_positions where link_id IN (' + ','.join(
        [str(id) for id in linkIds]) + ')'
    try:
        cursor.execute(queryStatement)
...
def loadTrajectoriesFromVissimFile(filename, simulationStepsPerTimeUnit, objectNumbers=None, warmUpLastInstant=None, usePandas=False, nDecimals=2, lowMemory=True):
    ...
    if usePandas:
        data = read_csv(filename, delimiter=';', comment='*',
                        header=0, skiprows=1, low_memory=lowMemory)
        ...
        for row_index, row in instants.iterrows():
            ...
            objects[objNum] = moving.MovingObject(
                num=objNum, timeInterval=moving.TimeInterval(row['first'], row['last']))
            # positions should be rounded to nDecimals decimals only
            objects[objNum].curvilinearPositions = moving.CurvilinearTrajectory(S=npround(tmp['POS'].tolist(
            ), nDecimals), Y=npround(tmp['POSLAT'].tolist(), nDecimals), lanes=tmp['LANE'].tolist())
        ...
        if filename.endswith(".fzp"):
                ...
                if objNum not in objects:
                    if warmUpLastInstant is None or instant >= warmUpLastInstant:
                        if objectNumbers is None or len(objects) < objectNumbers:
                            objects[objNum] = moving.MovingObject(
                                num=objNum, timeInterval=moving.TimeInterval(instant, instant))
                            objects[
                                objNum].curvilinearPositions = moving.CurvilinearTrajectory()
                if (warmUpLastInstant is None or instant >= warmUpLastInstant) and objNum in objects:
                    objects[objNum].timeInterval.last = instant
                    objects[objNum].curvilinearPositions.addPositionSYL(
                        s, y, lane)
                line = readline(inputfile, '*$')
            ...
            print("File type of " + filename +
                  " not supported (only .sqlite and .fzp files)")
...
def countStoppedVehiclesVissim(filename, lanes=None, proportionStationaryTime=0.7):
    ...
    if filename.endswith(".fzp"):
        ...
        data = read_csv(filename, delimiter=';', comment='*',
                        header=0, skiprows=1, usecols=columns, low_memory=lowMemory)
        ...
        # pb of the meaning of proportionStationaryTime in arterial network?
        # Why proportion of existence time?
        pass
    else:
        print("File type of " + filename +
              " not supported (only .sqlite and .fzp files)")
...
def countCollisionsVissim(filename, lanes=None, collisionTimeDifference=0.2, lowMemory=True):
    ...
    for name, group in merged.groupby(['LANE\LINK\NO', 'LANE\INDEX', 'NO_x', 'NO_y']):
        ...
        # diff = group['POS_x']-group['POS_y'] # to check the impact of
        # convert_objects and the possibility of using type conversion in
        # read_csv or function to convert strings if any
    ...
    # SQL select link_id, lane_id, id1, id2, min(diff), max(diff) from (select
    # CP1.t as t, CP1.link_id as link_id, CP1.lane_id as lane_id,
    # CP1.trajectory_id as id1, CP2.trajectory_id as id2, CP1.s_coordinate -
    # CP2.s_coordinate as diff from curvilinear_positions CP1,
    # curvilinear_positions CP2 where CP1.link_id = CP2.link_id and
    # CP1.lane_id = CP2.lane_id and CP1.t = CP2.t and CP1.trajectory_id >
    # CP2.trajectory_id) group by link_id, lane_id, id1, id2 having
    # min(diff)*max(diff) < 0
    return nCollisions
...
def loadTrajectoriesFromNgsimFile(filename, nObjects=-1, sequenceNum=-1):
    ...
    def createObject(numbers):
        ...
        obj = moving.MovingObject(num=int(numbers[0]),
                                  ...
                                  positions=moving.Trajectory(
                                      [[float(numbers[6])], [float(numbers[7])]]),
    ...
    for line in inputfile:
        ...
        if obj.getNum() != int(numbers[0]):
            ...
            obj.curvilinearPositions.addPositionSYL(
                float(numbers[5]), float(numbers[4]), obj.laneNums[-1])
...
class ProcessParameters(VideoFilenameAddable):
    ...
    def loadConfigFile(self, filename):
        ...
        self.databaseFilename = config.get(
            self.sectionHeader, 'database-filename')
        self.homographyFilename = config.get(
            self.sectionHeader, 'homography-filename')
        if (path.exists(self.homographyFilename)):
            self.homography = loadtxt(self.homographyFilename)
        else:
            self.homography = None
        self.intrinsicCameraFilename = config.get(
            self.sectionHeader, 'intrinsic-camera-filename')
        ...
        distortionCoefficients = getValuesFromINIFile(
            filename, 'distortion-coefficients', '=')
        self.distortionCoefficients = [
            float(x) for x in distortionCoefficients]
        self.undistortedImageMultiplication = config.getfloat(
            self.sectionHeader, 'undistorted-size-multiplication')
        ...
        self.speedAggregationMethod = config.get(
            self.sectionHeader, 'speed-aggregation-method')
        self.nFramesIgnoreAtEnds = config.getint(
            self.sectionHeader, 'nframes-ignore-at-ends')
        self.speedAggregationQuantile = config.getint(
            self.sectionHeader, 'speed-aggregation-quantile')
        self.minSpeedEquiprobable = config.getfloat(
            self.sectionHeader, 'min-speed-equiprobable')
        self.minNPixels = config.getint(self.sectionHeader, 'min-npixels-crop')
        self.pedBikeCarSVMFilename = config.get(
            self.sectionHeader, 'pbv-svm-filename')
        self.bikeCarSVMFilename = config.get(
            self.sectionHeader, 'bv-svm-filename')
        self.maxPedestrianSpeed = config.getfloat(
            self.sectionHeader, 'max-ped-speed')
        self.maxCyclistSpeed = config.getfloat(
            self.sectionHeader, 'max-cyc-speed')
        self.meanPedestrianSpeed = config.getfloat(
            self.sectionHeader, 'mean-ped-speed')
        self.stdPedestrianSpeed = config.getfloat(
            self.sectionHeader, 'std-ped-speed')
        self.locationCyclistSpeed = config.getfloat(
            self.sectionHeader, 'cyc-speed-loc')
        self.scaleCyclistSpeed = config.getfloat(
            self.sectionHeader, 'cyc-speed-scale')
        self.meanVehicleSpeed = config.getfloat(
            self.sectionHeader, 'mean-veh-speed')
        self.stdVehicleSpeed = config.getfloat(
            self.sectionHeader, 'std-veh-speed')

        # Safety parameters
        self.maxPredictedSpeed = config.getfloat(
            self.sectionHeader, 'max-predicted-speed') / 3.6 / self.videoFrameRate
        self.predictionTimeHorizon = config.getfloat(
            self.sectionHeader, 'prediction-time-horizon') * self.videoFrameRate
        self.collisionDistance = config.getfloat(
            self.sectionHeader, 'collision-distance')
        self.crossingZones = config.getboolean(
            self.sectionHeader, 'crossing-zones')
        self.predictionMethod = config.get(
            self.sectionHeader, 'prediction-method')
        ...
        self.useFeaturesForPrediction = config.getboolean(
            self.sectionHeader, 'use-features-prediction')
...
class SceneParameters(object):
    def __init__(self, config, sectionName):
        ...
        try:
            ...
            self.homographyFilename = config.get(
                sectionName, 'homography-filename')
            self.calibrationFilename = config.get(
                sectionName, 'calibration-filename')
            self.videoFilename = config.get(sectionName, 'video-filename')
            self.frameRate = config.getfloat(sectionName, 'framerate')
            self.date = datetime.strptime(
                config.get(sectionName, 'date'), datetimeFormat)  # 2011-06-22 11:00:39
            self.translation = literal_eval(
                config.get(sectionName, 'translation'))  # = [0.0, 0.0]
